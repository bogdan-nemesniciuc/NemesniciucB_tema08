...
namespace OpenTK_winforms_z02
{
    public partial class Form1 : Form
    {
        ...
        private SursaDeLumina light1; // Obiectul nostru nou
        ...
        //    // initializare lumina 1 (lumina secundara)
        //    light1 = new SursaDeLumina(LightName.Light1);
        //    // o pozitionam deasupra cubului (y pozitiv)
        //    light1.Position = new Vector4(0f, 70f, 0f, 1f);
        //    // o facem o culoare diferita (ex: galben difuz) pentru a o distinge
        //    light1.Diffuse = new Vector4(1.0f, 1.0f, 0.0f, 1.0f);
        ...
        private void SetupValues()
        {
            ...
            //  CONFIGURARE LIGHT 1 Sursa mobila
            light1 = new SursaDeLumina(LightName.Light1);
            light1.Position = new Vector4(0f, 30f, 0f, 1f); // Mai aproape de cub
            light1.Diffuse = new Vector4(1.0f, 1.0f, 0.0f, 1.0f); // Galben
            light1.Specular = new Vector4(1.0f, 1.0f, 1.0f, 1.0f); // Specular alb (pentru stralucire)

            // Setam atenuarea (Slide 10)
            // Formula: 1 / (Kc + Kl * d + Kq * d * d)
            // O atenuare liniara mica face lumina sa scada natural cu distanta
            light1.ConstantAttenuation = 0.0f;
            light1.LinearAttenuation = 0.02f;
            light1.QuadraticAttenuation = 0.0f;

            // O activam implicit pentru a vedea efectul
            light1.Enable();
        }
        ...
        {
            // Definim un material "plasticos" care reflecta lumina speculara

            // Componenta speculara a materialului (cat de mult reflecta stralucirea)
            float[] mat_specular = { 1.0f, 1.0f, 1.0f, 1.0f }; // Alb complet

            // Shininess (exponentul specular din modelul Phong)
            // Cu cat valoarea este mai mare, cu atat punctul de lumina este mai mic si mai concentrat.
        ...
        private void GlControl1_MouseMove(object sender, MouseEventArgs e) {
            ...
            // Logica NOUA pentru Lumina 1 (Click Dreapta)
            // Miscam lumina pe X si Y (planul ecranului)
            if (lightON && light1.IsEnabled && e.Button == MouseButtons.Right)
            {
                // Calculam diferenta fata de ultima pozitie sau o pozitie fixa
                // Aici facem o miscare simplificata bazata pe pozitia curenta vs centrul ferestrei sau delta
                // Pentru simplitate, folosim un factor de scalare fata de centrul controlului

                float factor = 0.2f; // Sensibilitate

                // Mutam lumina relativ la miscarea mouse-ului ar fi ideal, 
                // dar aici vom face un "follow" simplu pe X/Y

                // Varianta delta (mai corecta):
                // Ar trebui sa stocam lastMousePos. Dar pentru moment, facem update direct:

                // Modificam pozitia X si Y a luminii bazat pe coordonatele mouse-ului scalate
                float newX = (e.X - GlControl1.Width / 2) * factor;
                float newY = -(e.Y - GlControl1.Height / 2) * factor; // Inversam Y

                light1.Position = new Vector4(newX, newY, light1.Position.Z, 1.0f);
        ...
        {
            // functioneaza doar daca iluminarea globala e activa
            if (lightON)
            {
                light1.Toggle();
                btnLight1.Text = light1.IsEnabled ? "Sursa 1 (ON)" : "Sursa 1 (OFF)";
                GlControl1.Invalidate();
            }
        }
        ...
        {
            // Miscarea functioneaza doar daca Iluminarea globala si Sursa 1 sunt active
            if (lightON && light1.IsEnabled)
            {
                ...
                {
                    case Keys.U: // X+
                        light1.Move(speed, 0, 0);
                        break;
                    case Keys.J: // X-
                        light1.Move(-speed, 0, 0);
                        break;
                    case Keys.I: // Y+
                        light1.Move(0, speed, 0);
                        break;
                    case Keys.K: // Y-
                        light1.Move(0, -speed, 0);
                        break;
                    case Keys.O: // Z+
                        light1.Move(0, 0, speed);
                        break;
                    case Keys.L: // Z-
                        light1.Move(0, 0, -speed);
                        break;
                }
                GlControl1.Invalidate(); // Redesenare dupa miscare
            }
        ...
        private void GlControl1_Paint(object sender, PaintEventArgs e) {
            ...
            // --- COD NOU PENTRU LIGHT 1 ---
            if (lightON)
            {
                light1.Apply(); // Aceasta metoda din clasa face tot (Enable/Disable/Position/Color)

                // Vizualizare pozitie lumina (Optional: desenam un mic cubulet unde e lumina)
                if (light1.IsEnabled)
                {
                    GL.PushMatrix();
                    GL.Translate(light1.Position.X, light1.Position.Y, light1.Position.Z);
                    GL.Disable(EnableCap.Lighting); // Dezactivam iluminarea pt marker
                    GL.Color3(Color.Yellow);
                    // Desenare cub mic (marker sursa)
                    // Poti folosi o functie glutSolidSphere daca ai referinta, sau un cub manual mic
                    // Aici doar un punct mare pentru simplitate
                    GL.PointSize(10.0f);
                    GL.Begin(PrimitiveType.Points);
                    GL.Vertex3(0, 0, 0);
                    GL.End();
                    GL.Enable(EnableCap.Lighting); // Reactivam
                    GL.PopMatrix();
                }
            }
            else
            {
                // Daca iluminarea globala e oprita, ne asiguram ca si lumina 1 e oprita in OpenGL
        ...
        private void DeseneazaCubQ()
        {
            // 1. Activam ColorMaterial: Asta spune OpenGL-ului ca GL.Color3(...) 
            // reprezinta culoarea Difuza si Ambientala a materialului.
            GL.Enable(EnableCap.ColorMaterial);

            // 2. Aplicam proprietatile Speculare (stralucirea) definite in metoda ApplyMaterial
            ApplyMaterial();

            GL.Begin(PrimitiveType.Quads);
            for (int i = 0; i < nQuadsList; i++)
            {
                // Coloram fiecare vertex diferit pentru efect vizual
                switch (i % 4)
                {
                ...
                // 3. NORMALA: Esentiala pentru iluminare! 
                // Directia din care "respinge" suprafata lumina.
                // Aici folosim vectorul pozitie ca normala (aproximare pentru obiecte centrate in origine)
                GL.Normal3(arrVertex[x, 0], arrVertex[x, 1], arrVertex[x, 2]);

                // 4. VERTEXUL: Pozitia efectiva
                GL.Vertex3(arrVertex[x, 0], arrVertex[x, 1], arrVertex[x, 2]);
            }
            GL.End();

            // Dezactivam ColorMaterial pentru a nu afecta alte obiecte desenate ulterior
        ...
        {
            // 1. Activam ColorMaterial
            GL.Enable(EnableCap.ColorMaterial);

            // 2. Aplicam materialul lucios
            ApplyMaterial();

            GL.Begin(PrimitiveType.Triangles);
            for (int i = 0; i < nTrianglesList; i++)
            {
                // Coloram vertecșii (ciclul de 3 pentru triunghiuri)
                switch (i % 3)
                {
                    case 0: GL.Color3(Color.Blue); break;
                    case 1: GL.Color3(Color.Red); break;
